...
import {ArestaReta, GeometricForm, StopError} from './spiral.js';
...
export var keyReleased = function () {
	...
	if (key == 'a') {
		desenha (points);
		list_points.append (points.__getslice__ (0, null, 1));
		points = list ();
		last_points = list ();
	}
	if (key == 'z') {
		...
		if (size > 1) {
			stroke (160);
			line (last_points [size - 2] [0], last_points [size - 2] [1], last_points [size - 1] [0], last_points [size - 1] [1]);
			line (last_points [size - 2] [0], last_points [size - 2] [1], last_points [size - 1] [0], last_points [size - 1] [1]);
			line (last_points [size - 2] [0], last_points [size - 2] [1], last_points [size - 1] [0], last_points [size - 1] [1]);
			line (last_points [size - 2] [0], last_points [size - 2] [1], last_points [size - 1] [0], last_points [size - 1] [1]);
			line (last_points [size - 2] [0], last_points [size - 2] [1], last_points [size - 1] [0], last_points [size - 1] [1]);
...
export var draw = function () {
	...
	if (len (geometric) > 1) {
		for (var aresta of geometric.py_pop (0)) {
			stroke ('black');
...
export var desenha = function (points) {
	if (arguments.length) {
		var __ilastarg0__ = arguments.length - 1;
		if (arguments [__ilastarg0__] && arguments [__ilastarg0__].hasOwnProperty ("__kwargtrans__")) {
			var __allkwargs0__ = arguments [__ilastarg0__--];
	...
	var q1 = GeometricForm (arestas, minimal, tax);
	while (1) {
		try {
